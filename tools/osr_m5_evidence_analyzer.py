...
def cue_class(snippet):
    modern=[m for m in MODERN_MARKERS if m in snippet]
    classical=[m for m in CLASSICAL_MARKERS if m in snippet]
...
local_pair_examples=collections.defaultdict(list)
local_pair_shards=collections.defaultdict(set)
...
for r in rows:
    ...
    for a,b in itertools.combinations(local_present,2):
        key=(a,b); local_pair_shards[key].add((r.get('source'),r.get('file')))
        if len(local_pair_examples[key])<5:
            local_pair_examples[key].append({
                'source':r.get('source'),'file':r.get('file'),'row':r.get('row'),
                'row_sha256':r.get('row_sha256'),'local_terms':local_present,
                'cue_class':label,'modern_cues':modern,'classical_cues':classical,
                'snippet':snippet
            })
...
retention=collections.Counter((len(a),len(b)) for a,b in zip(row_sets,local_sets))
n=len(rows)
...
pair_examples=[]
for (a,b),count in local_pairs.most_common():
    pair_examples.append({
        'a':a,'b':b,'local_rows':count,'unique_shards':len(local_pair_shards[(a,b)]),
        'examples':local_pair_examples[(a,b)]
    })

analysis={
  'format':'osr-m5-evidence-analysis/v2.1','run_id':data.get('run_id'),'rows_found':n,
  'unique_row_hashes':len(unique_hashes),'exact_row_duplicate_fraction':round(1-len(unique_hashes)/n,6) if n else None,
  'repeated_snippet_rows':repeat_snippet_rows,'shards_with_evidence':len(shard_counts),
  ...
  'row_level':{
    'definition':'terms anywhere in the full source row/document','matched_term_count_histogram':{str(k):v for k,v in sorted(matched_hist.items())},
    'four_plus_term_rows':row_four_plus,'five_term_rows':row_five,
    'termset_counts':[{'terms':list(k),'rows':v} for k,v in row_termsets.most_common()],
    'pair_counts':[{'a':a,'b':b,'rows':v} for (a,b),v in row_pairs.most_common()],'pattern_counts':dict(row_patterns)
  },
  'local_window':{
    'definition':'terms physically present in the stored ~520-character snippet centered near the earliest hit',
    'two_plus_term_rows':local_two_plus,'four_plus_term_rows':local_four_plus,'five_term_rows':local_five,
    ...
    'pair_counts':[{'a':a,'b':b,'rows':v,'unique_shards':len(local_pair_shards[(a,b)])} for (a,b),v in local_pairs.most_common()],
    'pair_evidence_digest':pair_examples,'pattern_counts':dict(local_patterns),
  ...
  'interpretation_guardrail':'Row-level co-presence and local-window co-occurrence are different evidence strengths. Cue labels are heuristic only. Source identity, edition/date, genre, duplication, and independent-source audit are required before historical promotion.'
}
pathlib.Path('control/m5_evidence_analysis.json').write_text(json.dumps(analysis,ensure_ascii=False,indent=2)+'\n','utf-8')
print(json.dumps({'status':'PASS','rows':n,'local_pairs':len(local_pairs),'row_five':row_five,'local_five':local_five,'local_two_plus':local_two_plus},ensure_ascii=False))
